...
genRA2 = function(months, loanAmt, yearIntPC, payment){
    ...
    while(remBalance > 0){
        ...
        monthlyInterest = (remBalance * Number(monthlyRate)); //remBalance * monthlyRate/12

        totInterest = Number(totInterest ) + Number(monthlyInterest); //Add monthly interest to total interest
        //totInterest = Number(totInterest);

        monthlyPrincipal = (Number(payment) - Number(monthlyInterest));

        remBalance = (Number(remBalance) - Number(monthlyPrincipal));
...
/* 
        //================ if lastPayment is not equal to payment  
           if(payment > remBalance){ //damned type wondering about
            ...
            //if(currMon = months){ //damned type wondering about
                ...
                payment = Number(remBalance) + Number(monthlyInterest)
        ...
        temp.principal = Number(monthlyPrincipal).toFixed(2);
        temp.monthlyInterest = Number(monthlyInterest).toFixed(2); 
        ...
        console.log(`For currMon ${currMon} payment is ${payment} remBalance is ${remBalance} and monPrincipal is ${monthlyPrincipal}`);
    }
